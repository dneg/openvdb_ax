...
This library uses doxygen for creating its documentation. Following the instructions laid out in INSTALL, with the required dependencies this can be generated by calling the following command `make doc` in the root directory of the library folder.


### Getting up and running

Following building and installing the library as in INSTALL. If you have chosen to build the vdb_ax binary, this should be available from the library folder in the repository and can be accessed from the command line. Alternatively, use the OpenVDB AX Houdini SOP (coming soon) to modify OpenVDB data interactively within Houdini.
